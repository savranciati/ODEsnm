ode.mcmc_R7<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,7],dt)
    int.reg[,2]=trap.int(x[,6]*cos(x[,8]),dt)
    int.reg[,3]=trap.int(x[,2],dt)
    
    
    ### Sample beta
    var.beta=solve(S_beta*lam.beta+(t(int.reg)%*%int.reg)/sigma_d)
    m.beta=var.beta%*%(t(int.reg)%*%((y[,7]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,7]-rep(mu,n)-g1)%*%as.matrix(y[,7]-rep(mu,n)-g1)+(y[,7]-x[,7])%*%as.matrix(y[,7]-x[,7])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,7]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,7],dt)
  int.reg[,2]=trap.int(post.x[,6]*cos(post.x[,8]),dt)
  int.reg[,3]=trap.int(post.x[,2],dt)
...
require(compiler)
ode.mcmc_C7<-cmpfun(ode.mcmc_R7)
