ode.mcmc_R9<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,9],dt)
    int.reg[,2]=trap.int(x[,8]*sin(x[,10]),dt)
    ...
    m.beta=var.beta%*%(t(int.reg)%*%((y[,9]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,9]-rep(mu,n)-g1)%*%as.matrix(y[,9]-rep(mu,n)-g1)+(y[,9]-x[,9])%*%as.matrix(y[,9]-x[,9])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,9]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,9],dt)
  int.reg[,2]=trap.int(post.x[,8]*sin(post.x[,10]),dt)
...
require(compiler)
ode.mcmc_C9<-cmpfun(ode.mcmc_R9)
