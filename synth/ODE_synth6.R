ode.mcmc_R6<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    rt_y=(y[,6]-x[,1])%*%as.matrix(y[,6]-x[,1])
    shp_y=n
    ...
    int.reg[,1]=trap.int(x[,6],dt)
    int.reg[,2]=trap.int(x[,5]*cos(x[,7]),dt)
    int.reg[,3]=trap.int(x[,4],dt)
    
    
    ### Sample beta
    var.beta=solve(S_beta*lam.beta+(t(int.reg)%*%int.reg)/sigma_d)
    m.beta=var.beta%*%(t(int.reg)%*%((y[,6]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,6]-rep(mu,n)-g1)%*%as.matrix(y[,6]-rep(mu,n)-g1)+(y[,6]-x[,6])%*%as.matrix(y[,6]-x[,6])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,6]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,6],dt)
  int.reg[,2]=trap.int(post.x[,5]*cos(post.x[,7]),dt)
  int.reg[,3]=trap.int(post.x[,4],dt)
...
require(compiler)
ode.mcmc_C6<-cmpfun(ode.mcmc_R6)
