ode.mcmc_R1<-function(y,t, q,
  ...
  n=dim(y)[1]
  p=dim(y)[2]
  n.par=2 ### 2param each equation, a-b*sin(a) or -a+b*cos(a)
  n.par=n.par+1 ### identity functions for the other variables
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,1],dt)
    int.reg[,2]=trap.int(x[,10]*sin(x[,2]),dt)
    int.reg[,3]=trap.int(x[,5],dt)
  ...
  int.reg[,2]=trap.int(post.x[,10]*sin(post.x[,2]),dt)
  int.reg[,3]=trap.int(post.x[,5],dt)
...
require(compiler)
ode.mcmc_C1<-cmpfun(ode.mcmc_R1)
