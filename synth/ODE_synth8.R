ode.mcmc_R8<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,8],dt)
    int.reg[,2]=trap.int(x[,7]*sin(x[,9]),dt)
    int.reg[,3]=trap.int(x[,10],dt)
  
    
    
    ### Sample beta
    var.beta=solve(S_beta*lam.beta+(t(int.reg)%*%int.reg)/sigma_d)
    m.beta=var.beta%*%(t(int.reg)%*%((y[,8]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,8]-rep(mu,n)-g1)%*%as.matrix(y[,8]-rep(mu,n)-g1)+(y[,8]-x[,8])%*%as.matrix(y[,8]-x[,8])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,8]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,8],dt)
  int.reg[,2]=trap.int(post.x[,7]*sin(post.x[,9]),dt)
  int.reg[,3]=trap.int(post.x[,10],dt)
...
require(compiler)
ode.mcmc_C8<-cmpfun(ode.mcmc_R8)
