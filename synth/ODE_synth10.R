ode.mcmc_R10<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,10],dt)
    int.reg[,2]=trap.int(x[,9]*sin(x[,1]),dt)
    int.reg[,3]=trap.int(x[,3],dt)
    
    
    ### Sample beta
    var.beta=solve(S_beta*lam.beta+(t(int.reg)%*%int.reg)/sigma_d)
    m.beta=var.beta%*%(t(int.reg)%*%((y[,10]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,10]-rep(mu,n)-g1)%*%as.matrix(y[,10]-rep(mu,n)-g1)+(y[,10]-x[,10])%*%as.matrix(y[,10]-x[,10])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,10]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,10],dt)
  int.reg[,2]=trap.int(post.x[,9]*sin(post.x[,1]),dt)
  int.reg[,3]=trap.int(post.x[,3],dt)
...
require(compiler)
ode.mcmc_C10<-cmpfun(ode.mcmc_R10)
