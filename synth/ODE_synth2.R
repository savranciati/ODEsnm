ode.mcmc_R2<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,2],dt)
    int.reg[,2]=trap.int(x[,1]*sin(x[,3]),dt)
    ...
    m.beta=var.beta%*%(t(int.reg)%*%((y[,2]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,2]-rep(mu,n)-g1)%*%as.matrix(y[,2]-rep(mu,n)-g1)+(y[,2]-x[,2])%*%as.matrix(y[,2]-x[,2])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,2]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,2],dt)
  int.reg[,2]=trap.int(post.x[,1]*sin(post.x[,3]),dt)
...
require(compiler)
ode.mcmc_C2<-cmpfun(ode.mcmc_R2)
