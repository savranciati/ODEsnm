ode.mcmc_R5<-function(y,t, q,
  ...
  for(tt in 1:nit){
    ...
    int.reg[,1]=trap.int(x[,5],dt)
    int.reg[,2]=trap.int(x[,4]*cos(x[,6]),dt)
    int.reg[,3]=trap.int(x[,1],dt)
    
    
    
    ### Sample beta
    var.beta=solve(S_beta*lam.beta+(t(int.reg)%*%int.reg)/sigma_d)
    m.beta=var.beta%*%(t(int.reg)%*%((y[,5]-rep(mu,n))/sigma_d))
    ...
    rt_d=t(y[,5]-rep(mu,n)-g1)%*%as.matrix(y[,5]-rep(mu,n)-g1)+(y[,5]-x[,5])%*%as.matrix(y[,5]-x[,5])
    
    shp_d=n+n
    sigma_d=1/rgamma(1,shape=shp_d/2,rate=rt_d/2)
    
    mu=rnorm(1,mean=(y[,5]-g1)[1],sd=sqrt(sigma_d))
  ...
  int.reg[,1]=trap.int(post.x[,5],dt)
  int.reg[,2]=trap.int(post.x[,4]*cos(post.x[,6]),dt)
  int.reg[,3]=trap.int(post.x[,1],dt)
...
require(compiler)
ode.mcmc_C5<-cmpfun(ode.mcmc_R5)
