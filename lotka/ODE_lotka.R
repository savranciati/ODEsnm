ode.mcmc_R<-function(y,t, q,
  ...
  n=dim(y)[1]
  p=dim(y)[2]
  n.prod=choose(p,2) ### number of interactions between variables
  n.par=1+n.prod ### intercept+t+int(x_k)+interactions
  
  prior.lam_teta=matrix(rep(c(0.01,0.1),p),p,2,byrow=TRUE)
  prior.lam_beta=c(0.01,1)
  ...
  for(tt in 1:nit){
    ...
    z=x
    prodotti=apply(indici,1,function(x,z) {z[,x[1]]*z[,x[2]]}, z)
    int.reg[,2]=apply(prodotti,2,trap.int,dt)
